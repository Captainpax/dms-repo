...
| `SERVER_PORT` | Runtime port passed to `ArchipelagoServer.py`. Set to `3333` to match the Dockerfile exposure. | `38281` |
| `SERVER_PASSWORD` | Optional connection password for players. | (empty) |

The installation script also provisions a `.venv` virtual environment and writes `/home/container/start.sh`, which activates the environment and forwards the host, port, and optional password to `ArchipelagoServer.py`.
...
4. **Start the server.** The panel runs `bash /home/container/start.sh`, which in turn activates the virtual environment and execs the container's `StartAP` bootstrap. `StartAP` launches `python3 ArchipelagoServer.py` on the configured host/port.
